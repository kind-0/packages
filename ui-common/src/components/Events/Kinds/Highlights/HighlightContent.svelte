<script lang="ts">
    import type { NDKEvent, NDKArticle, NDKHighlight } from "@nostr-dev-kit/ndk";
...
<div class="flex flex-row justify-center items-stretch gap-4 md:font-serif leading-[1.8rem]">
    ...
    <div class="flex-grow">
        <EventContent
            ...
            note={contextWithHighlight}
            tags={(article??highlight)?.tags}
        />
    </div>
</div>
...
</style> note={contextWithHighlight} tags={(article??highlight)?.tags} />
    </div>
</div>
